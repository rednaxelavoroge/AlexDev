      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="mb-16 max-w-3xl">
          ...
          <h2 className="text-4xl sm:text-5xl font-bold text-white font-display tracking-tight leading-tight">
            Что мы создаём для вашего бизнеса
          </h2>
          <p className="mt-4 text-gray-400 text-lg">
            Мы продаём результат, а не технологии. Каждое направление решает конкретную
            бизнес-задачу — от первой гипотезы до масштабируемой системы.
          </p>
        </div>

        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {SERVICES.map((s) => (
            <article key={s.title} className="glass-card p-8 rounded-2xl flex flex-col">
              ...
              <h3 className="text-xl font-bold text-white font-display mb-3">{s.title}</h3>
              <p className="text-gray-400 text-sm leading-relaxed mb-5">{s.value}</p>

              <dl className="space-y-2.5 mb-6 text-sm">
                <div className="flex gap-2">
                  <dt className="text-gray-500 shrink-0">Кому:</dt>
                  <dd className="text-gray-300">{s.client}</dd>
                </div>
                <div className="flex gap-2">
                  <dt className="text-gray-500 shrink-0">Результат:</dt>
                  <dd className="text-teal-300">{s.outcome}</dd>
                </div>
              </dl>
        ...
        <div className="mt-12 flex flex-col sm:flex-row items-center justify-between gap-6 glass-panel rounded-2xl p-8">
          <p className="text-gray-300 text-lg">
            Не нашли нужную услугу? Давайте обсудим вашу задачу индивидуально.
          </p>
          <a
            href="#contact"
